...
#define LOG_PREFIX(L, buf) \
    do {time_t t = time(NULL); struct tm* lt = localtime(&t); if (lt) {\
        ...
        amos::TID tid = amos::Thread::CurrentThread();\
        sprintf(buf, "[%04d-%02d-%02d %02d:%02d:%02d.%03d.%03d][%s-%s:%d-%ld] ",\
                lt->tm_year + 1900, lt->tm_mon + 1, lt->tm_mday + 1,\
                lt->tm_hour + 1, lt->tm_min + 1, lt->tm_sec + 1, (int)tv.tv_usec/1000,\
                (int)tv.tv_usec%1000, L, __FILE__, __LINE__, (long)tid);}\
    }while(0)
...
#define LOG_DEBUG(...) \
    ...
    amos::log_print(__log_pre__, __log_buf__);}while(0)
